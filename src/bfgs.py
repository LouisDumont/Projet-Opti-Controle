...
def bfgs(oracle, x0, iter_max = 5000, threshold = 0.000001, visual=True, verbose=False):
    ...
    for k in range(iter_max):
        ...
        aux_xg = np.dot(delta_x, delta_grad)/np.vdot(delta_grad, delta_x)
        #print(aux_xg.size)
        aux_gx = np.dot(delta_grad, delta_x)/np.vdot(delta_grad, delta_x)
        #print(aux_gx.size)
        aux_xx = np.dot(delta_x, delta_x)/np.vdot(delta_grad, delta_x)
        #print(aux_xx.size)
